...
hubspotAPI.prototype.execute = function (verbParams, path, availableParams, givenParams, callback) {
  ...
  var json = '{"properties":[';
  var parameters = '';
  for (var i = 0; i < givenParams.length; i++) {
    console.log(givenParams[i]);
    parameters += '{"property":"'+givenParams[i].property+'", "value":"'+givenParams[i].value+'"},';
  }
  ...
  var uri = this.httpUri + '/' + path;// + '/?hapikey='+authParams.hapikey;
  requestOptions = {
    method: verb,
    headers: headers,
    encoding: null
  ...
  if (verb === "POST") {
      ...
      requestOptions[verbParams.json ? 'json' : 'form'] = json;
  ...
  requestOptions.uri = uri + "/?" + qs.stringify(authParams);
 ...
 */
 ...
 hubspotAPI.prototype.contacts_create_update = function(email, params, callback) {
  var version = params.version ? params.version : "v1";
